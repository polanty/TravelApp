...
AxiosError.from = (error, code, config, request, response, customProps) => {
  ...
  const msg = error && error.message ? error.message : 'Error'; // Prefer explicit code; otherwise copy the low-level error's code (e.g. ECONNREFUSED)

  const errCode = code == null && error ? error.code : code;
  AxiosError.call(axiosError, msg, errCode, config, request, response); // Chain the original error on the standard field; non-enumerable to avoid JSON noise
...
var _default = config => {
  ...
  newConfig.url = (0, _buildURL.default)((0, _buildFullPath.default)(newConfig.baseURL, newConfig.url, newConfig.allowAbsoluteUrls), config.params, config.paramsSerializer); // HTTP basic authentication
  ...
  if (_utils.default.isFormData(data)) {
    if (_index.default.hasStandardBrowserEnv || _index.default.hasStandardBrowserWebWorkerEnv) {
      ...
      const formHeaders = data.getHeaders(); // Only set safe headers to avoid overwriting security headers
    ...
    return _regenerator().w(function (_context) {
      while (1) switch (_context.p = _context.n) {
        ...
        case 2:
          _context.p = 2;
          _t = _context.v;
          showAlert('error', _t?.response?.data?.message || _t?.message || 'An unexpected error occurred');
  ...
  return _regenerator().w(function (_context2) {
    while (1) switch (_context2.p = _context2.n) {
      ...
      case 2:
        _context2.p = 2;
        _t2 = _context2.v;
        showAlert('error', _t2?.response?.data?.message || _t2?.message || 'An unexpected error occurred');
...
module.exports = function (locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoicG9sYW50eSIsImEiOiJjbWZpOHFnaW0waTI3MmpzZnQ5Y2Z6cTB4In0.U_EHyYbjhuHTeX3UP9O16g';
...
var _require = require('./alerts'),
    showAlert = _require.showAlert; // update Data
...
function () {
  ...
  /*#__PURE__*/
  _regenerator().m(function _callee2(data, type) {
    var url, res, _t2;
    ...
    return _regenerator().w(function (_context2) {
      while (1) switch (_context2.p = _context2.n) {
        ...
        case 3:
          _context2.p = 3;
          _t2 = _context2.v;
          showAlert('error', _t2.response.data.message);
...
/*eslint-disable*/
var axios = require('axios') && require('axios').default || require('axios');
      ...
      while (1) switch (_context.n) {
        case 0:
          if (!stripePromise) {
            ...
            _require3 = require('@stripe/stripe-js'), loadStripe = _require3.loadStripe;
            stripePromise = loadStripe('pk_test_51SWiKpEIA2zSyutJrRPeGXbONI3DgRe3lxM0BLMSRdF5setGFEi8PAjynRqzExuESGQTQZcfYEAYddGkIaJM9mne00jjstmlKR');
          }

          return _context.a(2, stripePromise);
      }
    }, _callee);
  }));
...
function () {
  var _ref2 = _asyncToGenerator(
  /*#__PURE__*/
  _regenerator().m(function _callee2(tourId) {
    var _ref4, data, stripe, _t;
    ...
    return _regenerator().w(function (_context2) {
      while (1) switch (_context2.p = _context2.n) {
        case 0:
          _context2.p = 0;
          _context2.n = 1;
          return axios.get("/api/v1/bookings/checkout-session/".concat(tourId));
        ...
        case 4:
          ...
          _t = _context2.v;
          console.error(_t);
          showAlert('error', _t.message); // alert(
...
var displayMap = require('./mapbox');

var updateData = require('./updateSettings').updateData;

var updateSettings = require('./updateSettings').updateSettings;

var _require2 = require('./stripe'),
    bookTour = _require2.bookTour; // +require('core-js/stable');
...
if (userDataForm) {
  userDataForm.addEventListener('submit', function (e) {
    ...
    form.append('photo', document.getElementById('photo').files[0]); // updateData(name, email);
